...
int **gFFTBitTable0 = NULL;
const int MaxFastBits0 = 16;

int IsPowerOfTwo0(int x)
{
	if (x < 2)
		return false;
...
int NumberOfBitsNeeded0(int PowerOfTwo)
{
	int i;
...
int ReverseBits0(int index, int NumBits)
{
	int i, rev;
...
void InitFFT0()
{
	gFFTBitTable0 = new int *[MaxFastBits0];
	
	int len = 2;
	for (int b = 1; b <= MaxFastBits0; b++) {
		
		gFFTBitTable0[b - 1] = new int[len];
		
		for (int i = 0; i < len; i++)
			gFFTBitTable0[b - 1][i] = ReverseBits0(i, b);
		
		len <<= 1;
	}
}

inline int FastReverseBits0(int i, int NumBits)
{
	if (NumBits <= MaxFastBits0)
		return gFFTBitTable0[NumBits - 1][i];
	else
		return ReverseBits0(i, NumBits);
}
         ...
         float *RealIn, float *ImagIn, float *RealOut, float *ImagOut)
{
	...
	NumBits = NumberOfBitsNeeded0(NumSamples);
...
void RealFFT0(int NumSamples, float *RealIn, float *RealOut, float *ImagOut)
{
	...
	FFT0(Half, 0, tmpReal, tmpImag, RealOut, ImagOut);
...
void PowerSpectrum0(int NumSamples, float *In, float *Out)
{
	int Half = NumSamples / 2;
	int i;
...
void fft0::powerSpectrum0(int start, int half, float *data, int windowSize,float *magnitude,float *phase, float *power, float *avg_power) {
	int i;
	int windowFunc = 3;
	...
	RealFFT0(windowSize, in_real, out_real, out_img);
...
void fft0::inversePowerSpectrum0(int start, int half, int windowSize, float *finalOut,float *magnitude,float *phase) {
	int i;
	int windowFunc = 3;
	...
	FFT0(windowSize, 1, in_real, in_img, out_real, out_img); // second parameter indicates inverse transform
